...
## Universal SARIMA vs. Pattern-Specific Forecasting

To evaluate whether pattern-specific routing provides measurable benefits over a
single universal model, we compared our proposed framework against a baseline
SARIMA model applied to all series. Results show consistent improvements across
all major forecasting metrics.

### Summary Table

| Metric   | Universal SARIMA | Pattern-Specific Routing | % Improvement |
|----------|------------------|---------------------------|--------------:|
...
```{=html}
<div align="center">
<img src="p1/pattern_specific_vs_universal.png" width="650">
<p style="font-size: 0.9em; color: #555;">
Figure: Direct comparison of forecasting accuracy between a universal SARIMA baseline
and the proposed pattern-specific routing. Lower values indicate better performance.
</p>
</div>
```
---
...
Figure summarizes the mean MAPE achieved by each model within the demand patterns where it could be applied. For intermittent demand, Croston delivers the lowest forecasting error, achieving substantially lower MAPE than SARIMA. This confirms Croston’s suitability for sparse series with zero-inflated structure, where explicitly modeling demand occurrence yields more stable forecasts.
...
While mean MAPE provides a summary measure of accuracy, it obscures the variability across individual store–item series. Figure presents MAPE boxplots for each model within its applicable demand patterns. The distributional patterns reinforce the conclusions above.

For intermittent demand, Croston not only achieves the lowest mean MAPE but also displays a tighter interquartile range than SARIMA, indicating more stable performance across series. SARIMA shows a wider spread, with several high-error outliers, which is consistent with the model’s sensitivity to long zero stretches.
...
To further compare model performance, Figure reports the number of store–item series for which each model achieved the lowest MAPE (“wins”). This metric highlights dominance within each demand pattern.
...
Accuracy conclusions are further validated through symmetric MAPE (sMAPE), which mitigates the inflation of percentage errors for low-volume series. As shown in the figure, the relative ordering of models remains consistent with the MAPE findings:
